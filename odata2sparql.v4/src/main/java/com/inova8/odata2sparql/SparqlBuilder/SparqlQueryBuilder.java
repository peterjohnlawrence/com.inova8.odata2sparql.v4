...
public class SparqlQueryBuilder {
	...
	public SparqlStatement prepareConstructSparql() throws EdmException, ODataApplicationException,
	...
	public SparqlStatement prepareCountEntitySetSparql() throws ODataApplicationException, EdmException,
			OData2SparqlException {
 ...
 private String getQueryOptionText(List<CustomQueryOption> queryOptions, String parameter){
	 ...
	 for ( CustomQueryOption queryOption: queryOptions){
		 if(queryOption.getName().equals(parameter)) return queryOption.getText();
	 }
	return null;
	...
	private StringBuilder clausesExpandSelect() throws EdmException, OData2SparqlException {
		...
		if (this.expandOption != null) {
			clausesExpandSelect.append(expandSelectTreeNodeWhere(rdfTargetEntityType.entityTypeName, this.expandOption,
					"\t"));
		}
		...
		if (rdfEntityType.isOperation()) {
			...
			if (segmentSize == 1) {
				...
				if (segmentSize > 2) {
					...
					RdfAssociation navProperty = rdfEntityType.findNavigationProperty(uriInfo.getUriResourceParts()
			...
			if (segmentSize > 2) {
				...
				RdfAssociation navProperty = rdfEntityType.findNavigationProperty(uriInfo.getUriResourceParts().get(1)
						.getSegmentValue());
				if (navProperty != null) {
					key = "?"
		...
		if (((UriResourceEntitySet) uriInfo.getUriResourceParts().get(0)).getKeyPredicates().size() != 0) {
			...
			for (UriParameter entityKey : ((UriResourceEntitySet) uriInfo.getUriResourceParts().get(0)).getKeyPredicates()) {
				...
				String expandedKey = rdfModel.getRdfPrefixes().expandPrefix(
						decodedEntityKey.substring(1, decodedEntityKey.length() - 1));
	...
	private StringBuilder clausesPathNavigation(String indent, List<UriResource> navigationSegments,
		...
		for (index = 1; index < segmentSize; index++) {
			...
			RdfAssociation navProperty = rdfModelToMetadata.getMappedNavigationProperty(new FullQualifiedName(predicate
					.getType().getNamespace(), predicate.getName()));
			if (isFirstSegment) {
	...
	private StringBuilder clausesExpandFilter(String indent) {
		...
		if (this.expandOption != null)
			clausesExpandFilter.append(expandSelectTreeNodeFilter(rdfEntityType.entityTypeName, this.expandOption,
					indent));
	...
	private StringBuilder expandSelectTreeNodeConstruct(String targetKey, ExpandOption expandTreeNode, String indent)
			throws EdmException {
	...
	private StringBuilder clausesSelect(String nextTargetKey, String navPath, String indent) {
		...
		//TODO Case URI5 need to fetch only one property as given in resourceParts
	...
	private StringBuilder expandSelectTreeNodeFilter(String targetKey, ExpandOption expandSelectTreeNode, String indent) {
	...
	private StringBuilder limitClause() {
		...
		//if (this.uriType.equals(UriType.URI1)||this.uriType.equals(UriType.URI6B)) {
		int top;
		int skip;
	...
	private HashMap<String, HashSet<String>> createSelectPropertyMap(SelectOption selectOption) throws EdmException {
		...
		if (selectOption != null) {
			for (SelectItem property : selectOption.getSelectItems()) {
				...
				// if so then 
				//V4			for (NavigationPropertySegment navigationPropertySegment : property.getNavigationPropertySegments()) {
				...
				if (property.isStar()) {
					...
					if (!property.getResourcePath().equals(RdfConstants.SUBJECT)) {
						...
						try {
							rdfProperty = segmentEntityType.findProperty(property.getResourcePath()
